...
from oeispy.sequences import A000058
from oeispy.sequences import A000073
from oeispy.sequences import A007908
from oeispy.sequences import A110396
...
def _evaluate_stack(s, state=None):
    ...
    if op == "Phi":
        ...
        if re.match(r"A\d{6}", op):
            function = importlib.import_module(f"oeispy.sequences.{op}")
        else:
            function = _fn[op]
        if sup:
